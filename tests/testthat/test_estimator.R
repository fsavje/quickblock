...
rep_te_estimator <- function(outcomes, blocking, treatments, t1, t0) {
  ok_unit <- !is.na(blocking) & !is.na(treatments)
  outcomes <- outcomes[ok_unit]
  blocking <- blocking[ok_unit]
  treatments <- treatments[ok_unit]

  n <- sum(!is.na(blocking))
  n_c <- sapply(split(blocking, blocking), length)
  mu1 <- sapply(split(outcomes[treatments == t1], blocking[treatments == t1]), mean)
  mu0 <- sapply(split(outcomes[treatments == t0], blocking[treatments == t0]), mean)
  sum((n_c / n) * (mu1 - mu0))
}

rep_var_estimator <- function(outcomes, blocking, treatments, t1, t0) {
  ok_unit <- !is.na(blocking) & !is.na(treatments)
  outcomes <- outcomes[ok_unit]
  blocking <- blocking[ok_unit]
  treatments <- treatments[ok_unit]

  r <- nlevels(treatments)
  n <- sum(!is.na(blocking))
  n_c <- sapply(split(blocking, blocking), length)
  z_c <- n_c %% r
  outcome_split1 <- split(outcomes[treatments == t1], blocking[treatments == t1])
  outcome_split0 <- split(outcomes[treatments == t0], blocking[treatments == t0])
  sigma1 <- ((n_c - 1) / n_c) * sapply(outcome_split1, function(x) { sum((x - mean(x))^2) / (length(x) - 1) })
  sigma0 <- ((n_c - 1) / n_c) * sapply(outcome_split0, function(x) { sum((x - mean(x))^2) / (length(x) - 1) })
  sum(
    (n_c^2 / n^2) * (
      (r / (n_c - 1)) +
        (r * z_c * (r - z_c) / ((n_c - 1) * (n_c - z_c) * (n_c + r - z_c)))
    ) * (sigma1 + sigma0)
  )
}

replica_block_est <- function(outcomes, blocking, treatments) {
  t_labels <- levels(treatments)
  out_te <- out_te_vars <- matrix(NA, nrow = nlevels(treatments), ncol = nlevels(treatments))
  dimnames(out_te) <- dimnames(out_te_vars) <- list(t_labels, t_labels)

  for (i in 1:nlevels(treatments)) {
    for (j in 1:nlevels(treatments)) {
      out_te[i, j] <- rep_te_estimator(outcomes, blocking, treatments, t_labels[i], t_labels[j])
      out_te_vars[i, j] <- rep_var_estimator(outcomes, blocking, treatments, t_labels[i], t_labels[j])
    }
  }

  if (any(is.nan(out_te_vars))) {
    out_te_vars[] <- NA
  }
  diag(out_te_vars) <- 0
...
my_outcomes <- c(0.0205, -0.3570, -0.1800,  1.6027,  0.6040,  0.0446,  1.1410,  0.6459,  0.2176,  1.0053,  0.8606, -1.6118,  0.5874,  1.1100,
...
df <- data.frame(x1 = raw_data[1:500], x2 = raw_data[501:1000])

set.seed(12345)

for(i in 1:5) {
  my_blocking1 <- quickblock(distances(df), size_constraint = 4L)
  my_treatments1 <- assign_treatment(my_blocking1)
  control_res <- replica_block_est(my_outcomes, my_blocking1, my_treatments1)
  package_res <- blocking_estimator(my_outcomes, my_blocking1, my_treatments1)
  test_that("`blocking_estimator` 1", {
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })


  my_blocking2 <- quickblock(distances(df), size_constraint = 2L)
  my_treatments2 <- assign_treatment(my_blocking2)
  control_res <- replica_block_est(my_outcomes, my_blocking2, my_treatments2)
  expect_warning(package_res <- blocking_estimator(my_outcomes, my_blocking2, my_treatments2))
  test_that("`blocking_estimator` 2", {
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })


  my_blocking3 <- quickblock(distances(df), size_constraint = 6L)
  my_treatments3 <- assign_treatment(my_blocking3, c("A", "B", "C"))
  control_res <- replica_block_est(my_outcomes, my_blocking3, my_treatments3)
  package_res <- blocking_estimator(my_outcomes, my_blocking3, my_treatments3)
  test_that("`blocking_estimator` 3", {
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })


  my_blocking4 <- quickblock(distances(df), size_constraint = 4L)
  my_treatments4 <- assign_treatment(my_blocking4, c("A", "B", "C"))
  control_res <- replica_block_est(my_outcomes, my_blocking4, my_treatments4)
  expect_warning(package_res <- blocking_estimator(my_outcomes, my_blocking4, my_treatments4))
  test_that("`blocking_estimator` 4", {
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })


  my_blocking5 <- quickblock(distances(df), size_constraint = 4L, caliper = 0.6)
  my_treatments5 <- assign_treatment(my_blocking5, c("A", "B"))
  control_res <- replica_block_est(my_outcomes, my_blocking5, my_treatments5)
  package_res <- blocking_estimator(my_outcomes, my_blocking5, my_treatments5)
  control_res$effects - package_res$effects
  control_res$effect_variances - package_res$effect_variances

  test_that("`blocking_estimator` 5", {
    #expect_equal(unname(package_res$effects), matrix(rep(0,4), ncol = 2))
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })


  my_blocking6 <- quickblock(distances(df), size_constraint = 4L, caliper = 0.6)
  my_treatments6 <- assign_treatment(my_blocking6, c("A", "B", "C"))
  control_res <- replica_block_est(my_outcomes, my_blocking6, my_treatments6)
  expect_warning(package_res <- blocking_estimator(my_outcomes, my_blocking6, my_treatments6))
  test_that("`blocking_estimator` 6", {
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })

  my_blocking7 <- quickblock(distances(df), size_constraint = 4L, caliper = 0.6)
  my_treatments7 <- assign_treatment(my_blocking7, c("A", "B", "C"))
  my_treatments7[is.na(my_treatments7)] <- "A"
  control_res <- replica_block_est(my_outcomes, my_blocking7, my_treatments7)
  expect_warning(package_res <- blocking_estimator(my_outcomes, my_blocking7, my_treatments7))
  test_that("`blocking_estimator` 7", {
    expect_equal(package_res$effects, control_res$effects)
    expect_equal(package_res$effect_variances, control_res$effect_variances)
  })
}
