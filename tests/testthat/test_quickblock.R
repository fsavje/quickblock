...
t_quickblock <- function(distances = distancesEucl,
                         size_constraint = 2L,
                         caliper = NULL,
                         ...) {
...
test_that("`quickblock` returns correct output", {
  ...
  expect_identical(t_quickblock(distances = raw_data),
                   t_sc_clustering(distances = distancesEucl))
  expect_identical(t_quickblock(distances = raw_data, normalize = "mahalanobize"),
                   t_sc_clustering(distances = distancesMaha))
  ...
  expect_identical(t_quickblock(primary_unassigned_method = "ignore"),
                   t_sc_clustering(primary_unassigned_method = "ignore"))
  expect_identical(t_quickblock(primary_radius = 0.5),
                   t_sc_clustering(primary_radius = 0.5))

  expect_warning(expect_identical(t_quickblock(seed_radius = 0.5, caliper = 2.0),
                                  t_sc_clustering(seed_radius = 0.5)))
  expect_identical(t_quickblock(caliper = 1.0, primary_unassigned_method = "ignore"),
                   t_sc_clustering(seed_radius = 0.5, primary_unassigned_method = "ignore", secondary_radius = "seed_radius"))
  expect_warning(expect_identical(t_quickblock(caliper = 1.0, primary_unassigned_method = "closest_assigned"),
                                  t_sc_clustering(seed_radius = 0.25, primary_unassigned_method = "closest_assigned")))
  expect_warning(expect_identical(t_quickblock(caliper = 1.0, primary_radius = "estimated_radius"),
                                  t_sc_clustering(seed_radius = 0.5, primary_radius = "estimated_radius", secondary_radius = "seed_radius")))

  expect_identical(t_quickblock(primary_data_points = c(2, 3, 4, 5, 6, 8, 10, 13, 15, 16, 18, 20, 21, 22, 23, 25, 27, 29, 32, 33, 34, 35, 36, 38, 39, 40, 41, 42, 45, 46, 47, 49,
                                                        50, 56, 57, 58, 60, 63, 64, 65, 70, 71, 75, 76, 77, 80, 82, 83, 88, 89, 95, 98, 99, 105, 106, 109, 110, 114, 116, 118, 123,
                                                        125, 130, 132, 134, 135, 136, 137, 138, 139, 141, 142, 143, 144, 147, 149, 153, 154, 156, 158, 159, 161, 162, 163, 164, 165,
                                                        168, 169, 172, 175, 179, 180, 182, 183, 184, 186, 187, 188, 189, 191, 192, 198, 199, 202, 203, 204, 205, 208, 209, 210, 211,
                                                        216, 218, 219, 221, 222, 223, 224, 225, 226, 229, 230, 232, 234, 235, 236, 237, 239, 253, 255, 256, 258, 266, 271, 273, 274,
                                                        278, 279, 280, 282, 283, 284, 285, 296, 297, 299, 301, 305, 306, 307, 308, 310, 313, 314, 315, 316, 317, 318, 321, 322, 324,
                                                        325, 328, 332, 334, 335, 336, 339, 340, 344, 345, 349, 350, 351, 353, 355, 360, 362, 365, 366, 367, 371, 372, 374, 375, 376,
                                                        382, 383, 384, 389, 390, 393, 394, 395, 396, 397, 399, 401, 402, 404, 405, 407, 408, 412, 413, 414, 416, 417, 418, 419, 424,
                                                        425, 426, 431, 432, 434, 435, 436, 437, 439, 441, 442, 445, 447, 448, 449, 452, 453, 458, 459, 461, 464, 465, 469, 474, 475,
                                                        476, 477, 478, 481, 484, 486, 488, 489, 490, 491, 494, 497, 498, 500)),
                   t_sc_clustering(primary_data_points = c(2, 3, 4, 5, 6, 8, 10, 13, 15, 16, 18, 20, 21, 22, 23, 25, 27, 29, 32, 33, 34, 35, 36, 38, 39, 40, 41, 42, 45, 46, 47, 49,
                                                           50, 56, 57, 58, 60, 63, 64, 65, 70, 71, 75, 76, 77, 80, 82, 83, 88, 89, 95, 98, 99, 105, 106, 109, 110, 114, 116, 118, 123,
                                                           125, 130, 132, 134, 135, 136, 137, 138, 139, 141, 142, 143, 144, 147, 149, 153, 154, 156, 158, 159, 161, 162, 163, 164, 165,
                                                           168, 169, 172, 175, 179, 180, 182, 183, 184, 186, 187, 188, 189, 191, 192, 198, 199, 202, 203, 204, 205, 208, 209, 210, 211,
                                                           216, 218, 219, 221, 222, 223, 224, 225, 226, 229, 230, 232, 234, 235, 236, 237, 239, 253, 255, 256, 258, 266, 271, 273, 274,
